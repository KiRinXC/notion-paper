...
10. 先按用户实际关注点重建章节骨架，再润色整个章节，包括最后的保留问题。保留用户的归类方式、判断、怀疑与证据边界。对技术页和流程页，从 PDF 及必要的官方代码/Artifact 补足使现有主题能够独立读懂的关键细节，包括输入、状态、逐步操作、训练或冻结对象、评价方式、输出及其用途；不得借此扩写用户尚未涉及的整章内容。
...
- 不主动补写用户尚未记录的整章内容；但用户已经记录某个技术机制或要求讲清流程时，必须补齐理解该机制所需的局部事实，不能只改写原有句子。
- 让成品在脱离当前对话和论文算法编号后仍能独立阅读；不要依赖“上面”“后面”“Algorithm 1/2”等预置上下文。
- 标题使用陈述性的名词短语或流程名称，不使用疑问句、算法编号或“我的判断/我的理解”等协作元标签充当通用标题。
- 普通变量、符号和短公式使用 Notion 行内公式，不用普通行内代码样式代替数学格式。
